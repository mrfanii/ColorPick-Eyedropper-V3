...
let dirMap;
let snapModeDelay = 3;
let errorScreenshotAttempts = 0;
let snapModeBlockedHere = false;
let snapModeTimeout = 0;
let snapExtPage = false;
let screenshotAlternativeRecieved = 0;
let realSrcRecieved = false;
let isScriptAlive = false;
let scriptAliveTimeout = 1;
let reExecutedNeedlessly = false;
let nbsp = '\u00A0';
let lastHex = 'FFF';
let isPopout = false;
let borderValue = '1px solid grey';
let EnableRGB = true;
let EnableHSL = true;
let useCSSValues = true;
let usePrevColorBG = false;
let showPreviousClr = true;
let pickEveryTime = false;
let closePopupOnTrigger = true;
let bbackgroundColor = 'white';
let hexHasHash = false;
let hexIsLowerCase = false;
let cpScaleOffset = isWindows ? 16 : 0;
let isPicking = false;
let keyInputMode = false;
let CSS3ColorFormat = loadedOptions.CSS3ColorFormat;
let snapModeBlock = loadedOptions.snapModeBlock;
let popupWaitTimeout = 4000;
let gotAnUpdate = false;
let EnableHex = true;
let otherError = false;
let fishEye = (localStorage['fishEye'] || pOptions['fishEye'].def) - 0;
...
function updateCurrentColor(r, g, b, justFields, omitId) {
  ...
  if (!justFields) {
    ...
    cp_set_from_hsv(hsv.h, hsv.s, hsv.v);
  }
}
...
function setButtonState(picking) {
  ...
  document.getElementById('epick').title =
    (isPicking ? chrome.i18n.getMessage('pickAgainUnlocked') : chrome.i18n.getMessage('pickAgainLocked')) +
    '\n( [' +
    chrome.i18n.getMessage('rightClick') +
    '] ' +
    (pickEveryTime ? chrome.i18n.getMessage('disablePickEveryTime') : chrome.i18n.getMessage('pickEveryTime')) +
    ' )';
  document.getElementById('pre').title =
    (isPicking ? chrome.i18n.getMessage('pickAgainUnlocked') : chrome.i18n.getMessage('pickAgainLocked')) + ' [' + chrome.i18n.getMessage('dblClick') + ']';
}
...
function wk(ev) {
  ...
  if (ev.keyCode == 27) {
    ...
    //r or j refresh
    resnap();
  } else if (!keyInputMode && ev.keyCode == 38) {
    //u
    movePixel(0, -1);
  } else if (!keyInputMode && ev.keyCode == 40) {
    //d
    movePixel(0, 1);
  } else if (!keyInputMode && ev.keyCode == 37) {
    //l
    movePixel(-1, 0);
  } else if (!keyInputMode && ev.keyCode == 39) {
    //r
    movePixel(1, 0);
  } else if (!keyInputMode && (ev.keyCode == 13 || ev.keyCode == 86)) {
    //enter, v
    toglPick();
    ...
    keyInputMode = true;
    stopPick();
    var rgb = fromHexClr(t.value);
    if (rgb) updateCurrentColor(rgb.r, rgb.g, rgb.b, false, t.id);
  } else if (t.id == 'crgb') {
    keyInputMode = true;
    stopPick();
    var comp = t.value.match(/\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/);
    if (comp) updateCurrentColor(comp[1] - 0, comp[2] - 0, comp[3] - 0, false, t.id);
  }
}
...
function iin() {
  ...
  if (document.getElementById('plat_prev')) {
    ...
    if (navigator.userAgent.indexOf('Windows') < 0) {
      ...
      } else document.getElementById('plat_prev').src = 'img/ico_mac.png';
    } else if (dow == 4) {
  ...
  if (window.location.href.indexOf('isPopup=') > -1) {
    ...
    chrome.windows.getCurrent({ windowTypes: ['popup'] }, function (win) {
      if (win.type == 'popup') {
        // shouldn't this check be redundant? is not
        popupWinId = win.id;
      }
    });
  } else {
...
function showErrorScreen(errorType) {
  ...
  if (lastErr && lastErr.ignore && lastErr.ignore[errorType]) {
    // to ignore subsequent errors of specified types
  ...
  if (err.chooser && !cp_chooser_booted) {
    init_color_chooser();
  }
  if (err.timer) {
    showLoadingTimer();
...
function beginSnapMode() {
  ...
  if (errorScreenshotAttempts > 5) {
    console.log('max err alternative screeshot attempts reached;');
    return;
  }
...
function tabScreenshotRecieved(dataUri, request) {
  ...
  Cr.elm(
    'a',
    {
      ...
      childNodes: [Cr.txt(chrome.i18n.getMessage('snapMode_prompt'))],
    },
...
function scriptsInjectedResult() {
  ...
  if (!isScriptAlive) {
    ...
    chrome.tabs.executeScript(tabid, { file: '/js/Cr.js' }, function () {
      if (chrome.runtime.lastError) {
        console.log(chrome.runtime.lastError.message);
        showErrorScreen('conent_port');
        return;
      }
      chrome.tabs.executeScript(tabid, { file: '/js/options_prefs.js' }, function () {
        chrome.tabs.executeScript(tabid, { file: '/js/colorpick.user.js' }, function () {
          isScriptAlive = true;
...
function finishSetup() {
  ...
  chrome.tabs.sendMessage(tabid, { enableColorPicker: true, historyLen: colorHistory.length }, function (response) {
    if (response.hex) updateCurrentColor(response.cr, response.cg, response.cb);
...
function mmove(ev) {
  if (isDrag) {
    ...
    (x = ev.pageX - window.pageXOffset), (y = ev.pageY - window.pageYOffset);
    (xm = Math.round((x1 - x) / fishEye)), (ym = Math.round((y1 - y) / fishEye));
    if (xm != 0 || ym != 0) {
...
function popupimage(mylink, windowname) {
  var w = Math.round(window.outerWidth * 1.114),
    h = Math.round(window.outerHeight * 1.15);
...
function cp_chooser_test_ready() {
  ++cp_loads;
  if (cp_loads >= cp_totalLoads) {
    cp_chooser_booted = true;
    cp_grad_render(cp_x != 0 || cp_y != 0);
  }
  ...
  Cr.elm(
    ...
    [
      Cr.elm('div', { id: 'chooser' }, [
        Cr.elm('div', { id: 'gradi_box' }, [Cr.elm('canvas', { id: 'gradi', width: '257', height: '256' })]),
      ...
      Cr.elm('div', { class: 'lbrow', id: 'hex-and-close' }, [
        ...
        Cr.elm('span', { id: 'hexrow' }, [Cr.elm('input', { type: 'text', spellcheck: 'false', id: 'hex', class: 'fullwidth' + (hexHasHash ? ' hex-hashed' : '') })]),
      ]),
      ...
      Cr.elm('div', { id: 'pres' }, [Cr.elm('div', { id: 'ohexpre' }), Cr.elm('div', { id: 'hexpre', title: chrome.i18n.getMessage('showColorChooser') })]),
      Cr.elm('div', { class: 'ctrls' }, [
        Cr.elm('a', { id: 'unreg_msg', target: '_blank', href: 'register.html', event: ['click', navToOptions], title: chrome.i18n.getMessage('buyRegisterTip') }, [
          /*Cr.txt(chrome.i18n.getMessage('registerBanner'))*/
        ]),
      ...
      Cr.elm('div', { id: 'ctrls', class: 'ctrls' }, [
        ...
        Cr.elm(
          'a',
          { target: '_blank', class: 'hilight', href: 'options.html', event: ['click', navToOptions], title: chrome.i18n.getMessage('configurationHelp'), id: 'optsb' },
