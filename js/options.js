...
function save_options() {
  ...
  smb.value = smb.value.replace(/^\||\|$/g, '');

  var i;

  for (i in pOptions) {
    document.getElementById(i).closest('label').style.border = '';
    if (typeof pOptions[i].def == 'boolean') localStorage[i] = document.getElementById(i).checked;
    else localStorage[i] = document.getElementById(i).value;
  }

  for (i in pAdvOptions) {
    document.getElementById(i).closest('label').style.border = '';
    if (typeof pAdvOptions[i].def == 'boolean') localStorage[i] = document.getElementById(i).checked;
    else localStorage[i] = document.getElementById(i).value;
  }
...
function restore_options() {
  var i;
  for (i in pOptions) {
    if (typeof pOptions[i].def == 'boolean') document.getElementById(i).checked = localStorage[i] == 'true' ? true : localStorage[i] == 'false' ? false : pOptions[i].def;
    else document.getElementById(i).value = localStorage[i] || localStorage[i] === '' ? localStorage[i] : pOptions[i].def;
  }
  ...
  for (i in pAdvOptions) {
    ...
    else document.getElementById(i).value = localStorage[i] || localStorage[i] === '' ? localStorage[i] : pAdvOptions[i].def;
  }
...
function toHex(d) {
  return ('00' + (d - 0).toString(16).toUpperCase()).slice(-2);
}
...
function test_rgb2hsv_to_hsv2rgb() {
  // 16777216 total possible rgb colors
  var matched = 0;
  var mismatched = 0;
  var wayMismatched = 0;
  var errPct = 0;
  ...
  console.log(
    ...
    ' mismatched and ',
    wayMismatched,
    ' wayMismatched with error percentage (0-1) computed as ',
    errPct,
    ' or ',
    errPct * 100,
    '% innaccurate',
...
function clear_history(ev) {
  if (confirm(chrome.i18n.getMessage('clear') + ' ' + chrome.i18n.getMessage('history') + '\n' + chrome.i18n.getMessage('deleteConfirm'))) {
    ...
    if (exiHisInner) {
      addUndo({
        // in the process of fragment creation, history is cleared
...
function currentSwatches() {
  ...
  if (colors.length < 1 && !exi) {
    swHld.appendChild(Cr.elm('div', { id: 'palette-help', childNodes: [Cr.txt(chrome.i18n.getMessage('noSwatches'))] }));
...
var dragMeta = {
  ...
  insertAfter: function (element, afterElm) {
...
function palleteSwatchEvents() {
  return [
    ['dragover', swatchDragOverEntry],
    ['dragleave', swatchDragOutEntry],
    ['drop', swatchDroppedEntry],
    ['dragend', swatchDroppedEntry],
    ['dragstart', swatchDragStart],
  ];
}
...
function createPalleteSwatch(hex, append) {
  ...
  return Cr.elm(
    'div',
    { class: 'swatch', draggable: true, style: 'background-color:' + hex + ';', events: palleteSwatchEvents() },
    [
        ...
        'div',
        {
          ...
          childNodes: [
            ...
            Cr.elm('span', {
              ...
              style: 'border:1px solid black;display:inline-block;width:1em;background-color:' + colorInput.value,
              title: colorInput.value,
              childNodes: [Cr.txt(nbsp)],
            }),
            Cr.txt(' '),
            Cr.elm('select', {
              id: 'palette-gen-mode',
  ...
  if (paletteGenData.Tones[palette_tone]) {
    ...
    for (t = 0, tl = toneList.length; t < tl; t++) {
      ...
      if ((toneResult.sat && toneResult.sat != 1.0) || (toneResult.val && toneResult.val != 1.0)) {
        // We already guarantee we have swatch entry above, so we handle any tonal or brightness modulations to the origional here
...
function add_all_history() {
  ...
  if (exiHisInner) {
    ...
    if (
      histories.length > 1 &&
      (paletteElms.length < 2 ||
        confirm(chrome.i18n.getMessage('addAllConfirm').replace('%history_count%', histories.length).replace('%palette_count%', paletteElms.length)))
    ) {
      for (var h = 0, hl = histories.length; h < hl; h++) {
...
function load_history() {
  ...
  Cr.elm(
    'div',
    {
      ...
      childNodes: [
        hist.length > 1 ? Cr.elm('a', { href: '#', style: 'font-size:10px;float:left;', event: ['click', clear_history] }, [Cr.txt(chrome.i18n.getMessage('clear'))]) : 0,
        hist.length > 1
          ? Cr.elm(
              'a',
              {
                ...
                events: [
                  [
                    'click',
                    function () {
                      alert(chrome.i18n.getMessage('trash_can') + '\n\n' + chrome.i18n.getMessage('trash_alt_click'));
                    },
                  ],
          ...
          : 0,
        hist.length > 2
          ? Cr.elm('a', { href: '#', style: 'font-size:10px;float:right;', event: ['click', add_all_history] }, [Cr.txt(chrome.i18n.getMessage('add_all'))])
          : 0,
      ],
    },
    historyInner,
...
function dragHistVrt(ev) {
  histReSizeVrt = true;
  disableSelection();
}
function dragHistBth(ev) {
  histReSize = histReSizeVrt = true;
  disableSelection();
}
function stopdragHist() {
  histReSize = histReSizeVrt = false;
  enableSelection();
}
function mmv(ev) {
...
function createOptions(piOptions, elemAppend) {
  ...
  for (i in piOptions) {
    ...
    if (piOptions[i].select) {
      ...
      cb.setAttribute('id', i);
      cb.setAttribute('size', piOptions[i].size || (piOptions[i].def + '').length);
      if (piOptions[i].ind > 0) l.appendChild(document.createTextNode(fourSpaces));
      l.appendChild(cb);
      l.appendChild(document.createTextNode(' ' + piOptions[i].name));
...
function createDailyColorViewer(container) {
  ...
  Cr.elm('div', {}, [Cr.txt(chrome.i18n.getMessage('colorOfDay')), Cr.elm('br', {}), Cr.elm('iframe', { id: 'ifcotd', src: '', scrolling: 'no' })], container);
}
  ...
  Cr.elm(
    ...
    [
      Cr.elm('h3', {}, [
        ...
        Cr.elm('a', { id: 'exthometxt', href: _ext_homepage, target: '_blank' }, [Cr.txt(chrome.i18n.getMessage('extName'))]),
        Cr.elm('br', {}),
        Cr.elm('br', {}),
        Cr.elm('br', {}),
      ]),
      ...
      Cr.elm('div', { id: 'swatch-holder' }, [
        Cr.elm('a', { href: '#', id: 'showpalette', class: 'toggleOpts' }, [Cr.elm('img', { src: 'img/expand.png' }), Cr.txt(chrome.i18n.getMessage('palette'))]),
        Cr.elm('div', { id: 'palette', class: 'indented-when-narrow-area' }, [
          Cr.elm('div', {
            ...
            childNodes: [Cr.elm('a', { class: 'swatchCtrl', event: ['click', clearSwatches] }, [Cr.txt(chrome.i18n.getMessage('clear').toLowerCase())])],
          }),
          ...
          Cr.elm('a', { class: 'swatchCtrl', event: ['click', printSwatches], style: 'float:' + dirMap.end + ';', target: '_blank', id: 'print-swatches' }, [
            Cr.txt(chrome.i18n.getMessage('printSave')),
          ]),
          Cr.elm('div', { id: 'swatches', style: 'display:block;position:relative;' }),
          Cr.elm('div', { id: 'generate-palette-area' }),
        ]),
      ]),
      Cr.elm('a', { href: '#', id: 'showhist', class: 'toggleOpts' }, [Cr.elm('img', { src: 'img/expand.png' }), Cr.txt(chrome.i18n.getMessage('history'))]),
      Cr.elm('div', { id: 'history', class: 'indented-area' }, []),
      Cr.elm('a', { href: '#', id: 'showopt', class: 'toggleOpts' }, [Cr.elm('img', { src: 'img/expand.png' }), Cr.txt(chrome.i18n.getMessage('options'))]),
      Cr.elm('div', { id: 'options', class: 'indented-area' }, []),
      Cr.elm('a', { href: '#', id: 'shoadvanc', class: 'toggleOpts' }, [Cr.elm('img', { src: 'img/expand.png' }), Cr.txt(chrome.i18n.getMessage('advancedOptions'))]),
      Cr.elm('div', { id: 'adv_options', class: 'indented-area' }, [
        Cr.elm('button', { id: 'bload' }, [Cr.txt(chrome.i18n.getMessage('fetchSync'))]),
        Cr.elm('button', { id: 'cload' }, [Cr.txt(chrome.i18n.getMessage('clearSync'))]),
      ]),
      Cr.elm('br'),
      Cr.elm('button', { id: 'bsave', class: 'options-btn' }, [Cr.txt(chrome.i18n.getMessage('saveOptions'))]),
      Cr.txt(' '),
      Cr.elm('button', { id: 'defa', class: 'options-btn' }, [Cr.txt(chrome.i18n.getMessage('showDefaults'))]),
      ...
      Cr.txt(' ' + chrome.i18n.getMessage('extName') + ' (V3)'),
    ],
    document.body,
  );
