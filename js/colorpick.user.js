var elmid1 = 'color_pick_click_box',
  elmid2 = 'ChromeExtension:Color-Pick.com';
...
var n = null,
  ...
  rgb = null;
hsv = null;
(scal = 1), (ex = 0), (ey = 0), (isEnabled = false), (isLocked = false), (msg_bg_unavail = chrome.i18n.getMessage('bgPageUnavailable'));
var ntx = {
...
var isMakingNew = false,
  lastNewTimeout = 0,
  snapshotLoadedTimeout = 0,
  imageRequestReachedBg = 0,
  pickHtmlUrl = chrome.runtime.getURL('pick.html');
var snapModeDetected = window.location.href.indexOf(pickHtmlUrl) === 0;
var msg_bg_unavail_snap = msg_bg_unavail,
  msg_error = 'Error',
  opts_url = pickHtmlUrl,
  msg_ext_name = 'ColorPick',
  ext_icon = pickHtmlUrl,
  ext_close = pickHtmlUrl;
...
function applyHexCase(hex) {
  return opts.hexIsLowerCase ? hex.toLowerCase() : hex;
}
function toHex(N) {
...
function reqLis(request, sender, sendResponse) {
  ...
  if (request.testAlive) {
    ...
    if (!sender || !sender.tab) {
      // if this was sent from a tab, it is from another content script, ignore
      disableColorPicker();
    }
  }
    ...
    Cr.elm(
      ...
      [
        ...
        Cr.elm('input', {
          ...
          readonly: true,
          size: 8,
          style: 'max-width:75px;font-size:10pt;border:' + opts.borderValue,
          id: 'cphexvl',
          value: (opts.hexHasHash ? '#' : '') + hex,
        ...
        opts.showPreviousClr && lhex != 'none'
          ? Cr.elm('input', {
              type: 'text',
              size: 1,
              style: 'max-width:50px;font-size:10pt;background-color:#' + lhex + ';border:' + opts.borderValue + ';border-left:none;',
              value: '',
            })
          : 0,
        opts.ShowRGBHSL && opts.EnableRGB && rgb
          ? Cr.elm('input', {
              type: 'text',
              readonly: true,
              style: 'max-width:150px;display:block;',
              value: 'rgb' + formatColorValuesWith(opts.CSS3ColorFormat, rgb.r, rgb.g, rgb.b),
              id: 'cprgbvl',
              event: ['mouseover', selectTargElm],
            })
          : 0,
        opts.ShowRGBHSL && opts.EnableHSL && hsv
          ? Cr.elm('input', {
              type: 'text',
              readonly: true,
              style: 'max-width:150px;display:block;',
              value: 'hsl' + formatColorValuesWith(opts.CSS3ColorFormat, hsv.h, hsv.s, hsv.v, 0, 1, 1),
              id: 'cphslvl',
              event: ['mouseover', selectTargElm],
            })
          : 0,
...
function setDisplay() {
  ...
  emptyNode(n);
  var iconStyles =
    'width:20px;height:20px;min-width:20px;min-height:20px;vertical-align:bottom;box-sizing:unset;box-shadow:none;background:unset;padding:8px;cursor:default;';
  ...
  Cr.elm(
    ...
    [
      ...
      Cr.elm('input', {
        ...
        readonly: true,
        size: 8,
        style: 'max-width:75px;font-size:10pt;vertical-align:bottom;padding:8px;border:' + opts.borderValue,
        id: 'cphexvl',
        value: (opts.hexHasHash ? '#' : '') + hex,
        event: ['mouseover', selectTargElm],
      }),
      Cr.elm('input', {
        ...
        role: 'img',
        style: iconStyles,
        src: chrome.runtime.getURL('img/icons/history/icon32.png'),
        title: chrome.i18n.getMessage('history'),
        id: 'cphistbtn',
        event: [
          'click',
          function (ev) {
            chrome.runtime.sendMessage({ goToOrVisitTab: 'options.html?history=' + hex }, function (r) {});
            ev.preventDefault();
          },
          true,
        ],
      ...
      Cr.elm('input', {
        ...
        role: 'img',
        style: iconStyles,
        src: chrome.runtime.getURL('img/icons/palette/icon32.png'),
        title: chrome.i18n.getMessage('generate_palette'),
        id: 'cpgenbtn',
        event: [
          'click',
          function (ev) {
            chrome.runtime.sendMessage({ goToOrVisitTab: 'options.html?palette=' + hex }, function (r) {});
            ev.preventDefault();
          },
          true,
        ],
      ...
      Cr.elm('a', {
        target: '_blank',
        href: 'https://vidsbee.com/ColorPick/Thanks/?search=%23' + hex + ' ' + fmt_rgb,
        childNodes: [Cr.elm('img', { style: iconStyles, src: chrome.runtime.getURL('img/icons/search/icon32.png') })],
      }),
      Cr.elm('input', {
        type: 'image',
        role: 'img',
        style: iconStyles,
        src: chrome.runtime.getURL('img/close.png'),
        title: chrome.i18n.getMessage('closeAndExit') + ' [esc]',
        id: 'cpexitbtn',
        event: ['click', dissableColorPickerFromHere, true],
      }),
      opts.ShowRGBHSL && opts.EnableRGB && rgb
        ? Cr.elm('input', { type: 'text', readonly: true, style: 'max-width:150px;display:block;', value: fmt_rgb, id: 'cprgbvl', event: ['mouseover', selectTargElm] })
        : 0,
      opts.ShowRGBHSL && opts.EnableHSL && hsv
        ? Cr.elm('input', {
            type: 'text',
...
function exitAndDetachWithMessage() {
  exitAndDetach();
  var detachedl = Cr.elm('img', {
    src: ext_icon,
    width: 64,
    align: 'top',
    style: 'vertical-align:top;display:inline-block;position:relative !important;padding: 0 0 5px 0 !important;margin:0 !important;',
  });
  var detachedct = Cr.elm('div', {
    id: 'detached-content',
    style: 'margin:14px;margin-left:164px;',
    childNodes: [
      ...
      Cr.elm('a', {
        href: opts_url + '?reveal=snapWaitTimeout',
        target: '_blank',
        event: [
          'click',
          function (ev) {
  ...
  var detached = Cr.elm(
    'div',
    {
      ...
      style:
        'position:fixed;bottom:0;right:25%;left:25%;cursor:default;z-index:2147483645;transition:0.5s ease-out;user-select:none;background:white;box-shadow:#000 0px 0px 10px 1px;font-family:sans-serif;color:#494949;font-size:14px;text-align:left;',
      events: [
        ['mousemove', moveNosnapNotice],
        ['mouseup', moveNosnapNotice],
      ],
      childNodes: [
        Cr.elm('img', {
          src: ext_close,
        ...
        Cr.elm('div', {
          style:
            "font-family:'Helvetica Neue','Lucida Grande',sans-serif;font-size:16px;color:black;font-weight:300;text-shadow:white 1px 1px 2px;line-height:24px;padding:5px;text-align:left;opacity:0.9;float:left;",
          childNodes: [
            detachedl,
            Cr.elm('div', {
              style: 'display:inline-block;width:85px;margin:8px 0 0 5px;',
              childNodes: [Cr.txt(msg_ext_name)],
            }),
          ],
        }),
        detachedct,
...
function exitAndDetach() {
  disableColorPicker();
  if (!snapModeDetected) {
    // cannot re-runs scripts in snap env
...
function wk(ev) {
  ...
  } else if (ev.keyCode == 82 || ev.keyCode == 74) {
    //r or j refresh
    ssf();
...
function mmf(ev) {
  ...
  if (!isLocked) {
    if (ev) {
      ...
      (ex = Math.round(lx * x_cvs_scale)), (ey = Math.round(ly * y_cvs_scale));
      updateColorPreview();
    }
  }
}
...
function initialInit() {
  ...
  if (!connectListener) {
    chrome.runtime.connect().onDisconnect.addListener(function () {
      console.log(
        'Sorry - ColorPick detected the extension has been reloaded.  This instance of the content script is now defunct. You may have to refresh the page to use ColorPick here!',
        msg_bg_unavail,
      );
      exitAndDetach();
    });
...
function prefsLoadedCompleteInit() {
  ...
  c = Cr.elm(
    'canvas',
    {
      ...
      role: 'img',
      style:
        'position:fixed;width:auto!important;height:auto!important;max-width:none!important;max-height:none!important;top:0px!important;left:0px!important;margin:0px!important;padding:0px!important;overflow:hidden;z-index:2147483646;cursor:' +
        crosshairCss(),
      events: [
        ['click', picked, true],
        [
  ...
  n = Cr.elm(
    'div',
    {
      id: elmid2,
      style:
        'position:fixed;min-width:30px;max-width:300px;min-height:30px;box-shadow:2px 2px 2px #999;transition:' +
        ntx.def +
        '; border:' +
        opts.borderValue +
        ';z-index:2147483646;cursor:default;padding:4px;color:#000',
    },
    [Cr.txt(' ')],
    document.body,
  );
...
function moveWm(ev, swappedFn, forceDifferentSwap) {
  ...
  var t = waterm.style.top.match(/^0/),
    r = waterm.style.right.match(/^0/),
    b = waterm.style.bottom.match(/^0/),
    l = waterm.style.left.match(/^0/);
  var cliw = waterm.clientWidth;
  var le = ev.offsetX < 10,
    re = ev.offsetX > cliw - 10;
  if (forceDifferentSwap) {
    if (waterm.getAttribute('data-last-swap') == 'hz') {
      le = false;
      re = false;
    } else {
      le = true;
...
function remainingInit() {
  if (!document.body.style) {
    // page isn't loaded enough yet... try again soon...
...
function keepWmAway(ev) {
  ...
  if (
    (nr.bottom > wr.top && nr.bottom < wr.bottom && nr.right > wr.left && nr.right < wr.right) ||
...
function newImage() {
  ...
  clearTimeout(snapshotLoadedTimeout);
  snapshotLoadedTimeout = setTimeout(
    function () {
      console.warn(
        'Sorry - ColorPick experienced issues while waiting for the snapshot - Reload the page in order to pick colors here.  Here is how many newImage requests reached bg page:',
    ...
    (opts.snapWaitTimeout || 6000) * (snapModeDetected ? 2 : 1),
  ); // max 6 second wait for image, attempt to prevent endless spin, double that time in snap mode
...
var gl = 0,
  ...
  snapTexture = 0,
  shaderProgram = 0,
  textureSampPosition = 0,
  fishEyeScalePosition = 0;
...
function handleRendering(quick) {
  ...
  if (opts.iconIsBitmap) {
    ...
    tctx.drawImage(
      icvs,
      startPoint - browseIconHalfWidth,
      startPoint - browseIconHalfWidth,
      srcBrowseIconWidth,
      srcBrowseIconWidth,
      0,
...
function initializeCanvas() {
  ...
  if (webGlAvail && opts.pixelatedPreview && opts.allowWebGl) {
    ...
    var fragShader =
      ...
      '\n' +
      ' vec2 get=ctr + (agl ' +
      (opts.lessFishEye ? '/ (fishEyeScale * 2.0)' : '* dis') +
      ');' +
      '\n' +
